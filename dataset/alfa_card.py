...
class AlfaTransactionDataset(Dataset):
    def __init__(self,
                 mlm,
                 user_ids=None,
                 seq_len=6,
                 ...
                 return_labels=False,
                 skip_user=False,
                 path_to_parquets='/home/jovyan/transactions/AlfaBattle-2.0/alfabattle2_train_transactions_contest/train_transactions_contest',
        ...
        self.path_to_parquets = path_to_parquets
        ...
        self.vocab = Vocabulary(adap_thres, target_column_name = 'flag')
    ...
    def user_level_data(self):
        ...
        if self.cached and path.isfile(fname):
            ...
            # change "User" to "app_id"
            unique_users = self.trans_table["app_id"].unique()
            columns_names = list(self.trans_table.columns)

            for user in tqdm(unique_users):
                user_data = self.trans_table.loc[self.trans_table["app_id"] == user]
    ...
    def prepare_samples(self):
        ...
        for user_idx in tqdm(range(len(trans_data))):
    ...
    def get_from_parquets(self, path_to_dataset, target_file, num_parts_to_preprocess_at_once=10, num_parts_total=50, has_target=True):
        print("Reading data from parquets")
        for step in tqdm(range(0, num_parts_total, num_parts_to_preprocess_at_once), desc="Reading transactions data"):
            if step == 0:
                print("reading 0-s parquet")
                transactions_frame = read_parquet_dataset_from_local(path_to_dataset, step, num_parts_to_preprocess_at_once, 
                                                             verbose=True)
            else:
                transactions_frame_add = read_parquet_dataset_from_local(path_to_dataset, step, num_parts_to_preprocess_at_once, 
                                                             verbose=True)
                transactions_frame = transactions_frame.append(transactions_frame_add, ignore_index = True)
                
        train = pd.read_csv(target_file)
        transactions_frame = transactions_frame.merge(train, on='app_id')
        
        self.nrows = transactions_frame.shape[0]
        log.info(f"read data : {transactions_frame.shape}")
    ...
    def get_csv(self, fname):
        ...
        if self.user_ids:
            ...
            data = data[data['app_id'].isin(self.user_ids)]
    ...
    def encode_data(self):
        ...
        path_to_dataset = self.path_to_parquets
        ...
        if self.cached and path.isfile(path.join(dirname, fname)):
            ...
            #self.trans_table = self.get_from_parquets(path_to_dataset, target_file)
        ...
        #data = self.get_from_parquets(path_to_dataset = path_to_dataset, target_file = target_file)
        log.info(f"{data_file} is read.")

        #log.info("nan resolution.")
        #data['Is Fraud?'] = self.fraudEncoder(data['Is Fraud?'])
        #data['amnt'] = self.amountEncoder(data['amnt'])

        sub_columns = ['operation_kind', 'card_type', 'operation_type', 'operation_type_group', 'ecommerce_flag', 'payment_system', 'income_flag', 'mcc', 'country', 'city', 'day_of_week', 'hour', 'weekofyear', 'mcc_category']
        
        log.info("label-fit-transform.")
        for col_name in tqdm(sub_columns):
        ...
        time_cols = ['days_before', 'hour_diff']

        log.info("timestamp quant transform")
        for col_name in tqdm(time_cols):
        ...
        coldata = np.array(data['amnt'])
        bin_edges, bin_centers, bin_widths = self._quantization_binning(coldata)
        data['amnt'] = self._quantize(coldata, bin_edges)
        self.encoder_fit["amnt-Quant"] = [bin_edges, bin_centers, bin_widths]

        columns_to_select = ['app_id', 'operation_kind', 'card_type', 'operation_type', 'operation_type_group', 'ecommerce_flag', 'payment_system', 'income_flag', 'mcc', 'country', 'city', 'day_of_week', 'hour', 'weekofyear', 'mcc_category', 'days_before', 'hour_diff', 'amnt', 'flag']
        
        #, 'card_type', 'operation_type', 'operation_type_group', 'ecommerce_flag', 'payment_system', 'income_flag', 'mcc', 'country', 'city', 'day_of_week', 'hour', 'weekofyear', 'mcc_category', 'amnt', 
